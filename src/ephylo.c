...
SEXP C_ephylo_track_descendants(
    SEXP r_num_tips,
    SEXP r_num_nodes,
    SEXP r_postorder,
    SEXP r_left_child,
    ...
    SEXP r_right_sib)
{
    ...
    SEXP r_left_tip = PROTECT(Rf_allocVector(INTSXP, num_nodes));
    SEXP r_right_tip = PROTECT(Rf_allocVector(INTSXP, num_nodes));
    SEXP r_next_tip = PROTECT(Rf_allocVector(INTSXP, num_tips));

    const int *restrict nodes = INTEGER(r_postorder);
    const int *restrict left_child = INTEGER(r_left_child);
    const int *restrict right_child = INTEGER(r_right_child);
    const int *restrict right_sib = INTEGER(r_right_sib);
    int *restrict left_tip = INTEGER(r_left_tip);
    int *restrict right_tip = INTEGER(r_right_tip);
    int *restrict next_tip = INTEGER(r_next_tip);
    ...
    for (int i = 0; i < num_nodes; ++i)
    {
        ...
        {
            left_tip[u] = left_tip[left_child[u]-1];
            right_tip[u] = right_tip[right_child[u]-1];
            for (v = left_child[u]-1; v != -1; v = right_sib[v]-1)
            {
                if (right_sib[v] != 0)
                {
                    next_tip[right_tip[v]-1] = left_tip[right_sib[v]-1];
                }
                else
                {
                    next_tip[right_tip[v]-1] = 0;
                }
            }
        }
    }
